...
void GameManager<T>::run() {
    ...
    while (!boardPtr->game_is_over()) {
        for (int i : {0, 1}) {
            system("cls");  // Clear the screen at the start of each player's turn
            boardPtr->display_board();  // Display the board again after clearing the screen
            players[i]->getmove(x, y);
            while (!boardPtr->update_board(x, y, players[i]->getsymbol())) {

                RandomPlayer<T>* ai_player = dynamic_cast<RandomPlayer<T>*>(players[i]);
                // If it is not a RandomPlayer (i.e., a human player), prompt for a valid move
                if (ai_player == nullptr) {
                    cout << "Not empty cell. Please enter an empty cell index:" << endl;
                }
                players[i]->getmove(x, y);
            }
            boardPtr->display_board();
